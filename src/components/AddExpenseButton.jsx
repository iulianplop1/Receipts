...
export default function AddExpenseButton({ show, onClose, onAdd, userId }) {
  ...
  const [totalValidation, setTotalValidation] = useState(null) // { expected: number, calculated: number, isValid: boolean }
    ...
    try {
      ...
      for (const fileEntry of filesWithMeta) {
        ...
        try {
          ...
          const extractedTotalAmount = result?.total_amount ? parseFloat(result.total_amount) : null

          // Validate total amount
          if (extractedTotalAmount !== null) {
            const calculatedTotal = normalizedItems.reduce((sum, item) => sum + parseFloat(item.amount || 0), 0)
            const isValid = Math.abs(calculatedTotal - extractedTotalAmount) <= 0.02
            setTotalValidation({
              expected: extractedTotalAmount,
            ...
            if (!isValid) {
              console.warn(`Total amount mismatch for ${file.name}: Extracted items sum to ${calculatedTotal.toFixed(2)}, but receipt total is ${extractedTotalAmount.toFixed(2)}`)
            }
          } else {
            setTotalValidation(null)
          }
  ...
  const handleAddItem = () => {
    setReviewItems(prevItems => [
      ...prevItems,
      {
        ...
        category: 'Other',
        currency: 'DKK',
        date: bulkDate || getTodayDateString(),
        receiptFileId: null,
  ...
  const handleReanalyzeReceipt = async () => {
    ...
    try {
      ...
      for (const fileEntry of receiptFiles) {
        const { file } = fileEntry
        try {
          // Re-analyze with enhanced prompt for price correction
          const result = await analyzeReceipt(file)
          const rawItems = Array.isArray(result?.items)
            ? result.items
            : Array.isArray(result)
          ...
          console.error(`Error re-processing file ${file.name}:`, fileError)
        }
      }

      if (collectedItems.length === 0) {
        setError('No valid items found after re-analysis.')
        return
      }
      ...
      const errorMsg = err.message || 'Failed to re-analyze receipt.'
      if (errorMsg.includes('overloaded')) {
        setError('The AI service is currently busy. Please wait a moment and try again.')
      } else {
        setError(errorMsg)
      ...
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        {mode === 'review' && (
          <div className="add-expense-review">
            ...
            <p className="modal-subtitle">Edit items, add new ones, or remove incorrect entries. We found {reviewItems.length} item(s).</p>
            ...
            {totalValidation && (
              <div className={`price-validation ${totalValidation.isValid ? 'valid' : 'invalid'}`}>
                <div className="validation-header">
                  <span className="validation-icon">
                    {totalValidation.isValid ? '✅' : '⚠️'}
                  </span>
                  <span className="validation-title">
                    {totalValidation.isValid ? 'Prices Match Receipt Total' : 'Price Mismatch Detected'}
                  </span>
                </div>
                <div className="validation-details">
                  <div className="validation-item">
                    <span>Receipt Total:</span>
                    <span>{totalValidation.expected.toFixed(2)} DKK</span>
                  </div>
                  <div className="validation-item">
                    <span>Items Total:</span>
                    <span>{totalValidation.calculated.toFixed(2)} DKK</span>
                  </div>
                  {!totalValidation.isValid && (
                    <div className="validation-actions">
                      <button
                        className="btn-secondary reanalyze-button"
                        onClick={handleReanalyzeReceipt}
                        disabled={loading}
                      >
                        {loading ? 'Re-analyzing...' : '🔄 Re-analyze Receipt'}
                      </button>
                      <small>AI will try to fix price calculations</small>
                    </div>
                  )}
                </div>
              </div>
            ...
            <button 
              className="btn-secondary add-item-button"
