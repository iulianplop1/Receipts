...
// Fallback function using Grok API when Gemini is overloaded
async function analyzeReceiptWithGrok(imageFile) {
  try {
    console.log('Attempting to analyze receipt with Grok API...')
...
Return a JSON object with this exact structure:
{
  ...
  "total_amount": 0.00 // The total amount of the entire receipt, if available
}
...
- Looking for discount information near prices`
    ...
    // Use Grok API with vision capabilities
    ...
    if (!jsonObjectMatch) {
      throw new Error('Could not parse receipt data from Grok response')
    }
    ...
    if (receiptDate) {
      const today = new Date()
      const maxPastDate = new Date(today.getFullYear() - 2, today.getMonth(), today.getDate())
    ...
    const extractedTotalAmount = parseFloat(parsed.total_amount) || null

    console.log('Successfully analyzed receipt with Grok API')
    return {
      date: receiptDate,
      items: filteredItems,
      total_amount: extractedTotalAmount,
    }
...
Receipt shows: "Apple 2 stk @ 10.00 DKK", Total: "20.00 DKK"
Return:
{
  "date": "2024-01-15",
  "items": [
...
- Looking for discount information near prices`
    ...
    try {
      ...
      if (jsonArrayMatch) {
        ...
        return {
          ...
          items: filteredItems,
          total_amount: null, // No total amount if only array is returned
        }
      }
  ...
  // If all Gemini models failed, try Grok as fallback
  console.error('All Gemini models failed. Last error:', lastError)
  const errorMessage = lastError?.message || 'Unknown error'

  // Check if this is a rate limit error - if so, try Grok
  if (errorMessage.includes('429') || errorMessage.includes('quota') || errorMessage.includes('overloaded') || errorMessage.includes('resource exhausted')) {
    console.log('Gemini is rate limited, attempting fallback to Grok API...')

    if (GROK_API_KEY && GROK_API_KEY !== '') {
      try {
        return await analyzeReceiptWithGrok(imageFile)
      } catch (grokError) {
        console.error('Grok fallback also failed:', grokError)
        throw new Error('Both Gemini and Grok AI services are currently unavailable. Please try again later.')
      }
    } else {
      throw new Error('Gemini AI service is currently overloaded and no Grok API key is configured. Please wait a moment and try again.')
    }
  }

  // For other types of errors, don't try Grok fallback
